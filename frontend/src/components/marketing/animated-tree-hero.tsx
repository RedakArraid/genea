...
function MiniPersonCard({
  ...
  const lifespan = person.born ? (person.died ? `${person.born}–${person.died}` : `${person.born}–`) : "?"
...
export function AnimatedTreeHero({ className }: { className?: string }) {
  ...
  const { w: cardW, h: cardH } = getCardDimensions("square")

  const { people, positions, connections, bounds } = useMemo(() => {
    const normalized = normalizePersons(MARKETING_DEMO_PERSONS, MARKETING_DEMO_RELATIONSHIPS)
    const { positions: pos } = computeLayout(normalized, "vertical", "spacious")
  ...
  return (
    ...
    >
      ...
      >
        <div
          className="relative origin-center scale-[0.72] sm:scale-[0.78] lg:scale-[0.85]"
          ...
          {people.map((p) => {
            ...
            return (
              <MiniPersonCard
                ...
                w={cardW}
                h={cardH}
                selected={p.id === MARKETING_HIGHLIGHT_PERSON_ID}
              />
            )
          })}
        </div>
